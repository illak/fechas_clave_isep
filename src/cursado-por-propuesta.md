...
<div class="hero">
  <h1>Cursado por propuesta y cohorte</h1>
  Conocé qué unidades curriculares o propuestas de acreditación única se cursan por propuesta y cohorte seleccionadas.
</div>
...
<div class="grid grid-cols-1" style="margin-top: 25px">
  ...
  let propuesta = view(Inputs.select([null].concat(propuestas_b), {label: "Propuesta", width:860}));
  ```
...
<div class="grid grid-cols-4" >
  <div class="card grid-colspan-4" id="tabla">
...
```js
  ...
  const selects = view(Inputs.table(dataFiltered, {
      ...
      header: {
        ...
        "TOTAL DE AULAS NO ASOCIADAS": "#Aulas",
      ...
      format: {
        "Propuesta": (d) => wrapText(d, 290),
        "Criterio de carga" : (d) => wrapText(d, 150),
        "tipo_ed": (d) => wrapText(d, 150),
        "TOTAL DE AULAS NO ASOCIADAS": (d) =>  wrapText(d,50),
        //"id": (d) => wrapText(d,220),
        id: id => {
